...
class AmbiguousTableAndGraphCorrelation(DirectInput):
    ...
    def generate(self):
        ...
        self.text = f'''На рисунке справа схема дорог Н-ского района изображена в виде графа, в таблице содержатся
сведения о дорогах между населенными пунктами (звездочка означает, что дорога между соответствующими городами есть). 
{g_for_table.html_matrix()} {html.div_xy(g.as_svg(), graph_data['size']['x'], graph_data['size']['y'], margin='5px')} Так как таблицу и схему рисовали 
независимо друг от друга, то нумерация населённых пунктов в таблице никак не связана с буквенными обозначениями на 
графе. Определите номера населенных пунктов {first_vertex} и {last_vertex} в таблице. В ответе запишите числа в порядке 
